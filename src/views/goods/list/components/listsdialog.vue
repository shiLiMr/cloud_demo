<template>
    <el-drawer v-model="drawer" size="800px" :title="isTitle" direction="rtl" :before-close="handleClose">
        <el-form class="list-dialog" :model="form" label-width="90px" label-suffix=" :" inline status-icon ref="ruleformRef" :rules="rules">
            <el-form-item label="商品名称" prop="name">
                <el-input v-model="form.name" placeholder="请输入商品名称" show-word-limit maxlength="30" clearable @blur="handleBlur" />
            </el-form-item>
            ...
            <el-form-item label="库存数量" prop="stockNum">
                <el-input-number style="width: 100%;" v-model="form.stockNum" placeholder="请输入库存数量" :precision="2" :min="1" clearable />
            </el-form-item>
            ...
            <el-form-item label="商品特价" prop="discountPrice">
                <el-input-number style="width: 100%;" placeholder="0.00" v-model="form.discountPrice" :precision="2" :min="1"  />
            </el-form-item>
            ...
            <el-form-item label="获得积分" prop="score">
                <el-input-number style="width: 100%;" placeholder="请输入获得积分" v-model="form.score" :precision="2" :min="1" />
            </el-form-item>
...
import { getGoodsCategoryAddApi ,getGoodsCategoryEditApi} from '@/api/goods/list'
...
const openDrawer = (type:string,title:string,data={} as any) => {
    ...
    if(type==='edit'){
        nextTick(()=>{
            ...
            // form.value.goodsDetail= `<p>${data.goodsDetail}</p>`
        })
    }
}
// 暴露子组件方法
